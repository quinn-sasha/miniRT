...
static void	set_up_camera(t_program *data)
{
	...
	lookfrom = init_vec3(13, 2, 3);
	lookat = init_vec3(0, 0, 0);
	view_up = init_vec3(0, 1, 0);
	focus_dist = 10.0;
	camera = init_camera(lookfrom, lookat, view_up, (double)WIDTH / HEIGHT, 45,
			0.1, focus_dist);
	data->camera = camera;
}
...
int	render(t_program *data)
{
	...
	for (int y = 0; y < HEIGHT; y++)
	{
		...
		for (int x = 0; x < WIDTH; x++)
		{
			...
			{
				x_offset = (x + random_double(&state)) / (WIDTH - 1);
				y_offset = (y + random_double(&state)) / (HEIGHT - 1);
				ray = get_ray(data->camera, x_offset, y_offset, &state);
				pixel_color = add_vec3(pixel_color, calculate_color(ray, data,
							&state, max_recursions));
			}

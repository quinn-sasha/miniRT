...
t_camera init_camera(t_vec3 look_from, t_vec3 look_at, t_vec3 view_up,
  ...
  camera.vertical =
      scale_vec3(camera.down_dir, viewport_height * focus_distance);
...
t_ray get_ray(t_camera camera, double x_offset, double y_offset,
  ...
  t_vec3 origin_offset = add_vec3(scale_vec3(camera.right_dir, pos_in_circle.x),
                                  scale_vec3(camera.down_dir, pos_in_circle.y));
  ...
  t_ray ray = init_ray(ray_origin, sub_vec3(offset_from_top_left, ray_origin));
  return ray;
}

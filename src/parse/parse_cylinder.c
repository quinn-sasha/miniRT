...
static int	parse_attrs(char **params, t_vec3 *vectors, double *dimensions)
{
	...
	if (parse_unit_vector(params[2], &vectors[1]) == PARSE_FAILED)
		return (PARSE_FAILED);
	if (parse_double_range(params[3], &dimensions[0], 0,
			INFINITY) == PARSE_FAILED)
		return (PARSE_FAILED);
	if (parse_double_range(params[4], &dimensions[1], 0,
			INFINITY) == PARSE_FAILED)
		return (PARSE_FAILED);
...
static int	parse_cylinder_helper(const char *line, t_program *data)
{
	...
	if (parse_color(params[5], &albedo) == PARSE_FAILED)
		return (handle_parse_error(params));
	cylinder = new_cylinder(vectors[0], vectors[1], dimensions[0] / 2,
			dimensions[1], parse_material(params[6], albedo));
